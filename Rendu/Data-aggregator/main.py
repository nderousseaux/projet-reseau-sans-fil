...
def analyse_nb_senders(path):
	...
	# On fait un histogramme du délai du réseau
	plt.bar([str(s.nbSenders) for s in scenarios], [s.get_mean_delay() for s in scenarios])

	# Titre
	plt.title("Délai du réseau en fonction du nombre de senders")

	# On affiche les labels
	plt.xlabel("Nombre de senders")
	plt.ylabel("Délai du réseau (ms)")
...
def analyse_size_slot(path):
	...
	plt.show()
		...
		# On fait un histogramme du délai du réseau
	plt.bar([str(s.slotSize) for s in scenarios], [s.get_mean_delay() for s in scenarios])

	# Titre
	plt.title("Délai du réseau en fonction de la taille des slots")
